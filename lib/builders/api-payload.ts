...
const PROFILE_CATEGORY: Record<string, number> = {
  E1A: 0, E1B: 1, E2A: 2, E2B: 3, E3A: 4, E3B: 5, E4A: 6, E4B: 7,
  G1A: 8, G2A: 9, G2B: 10, G2C: 11, GXX: 12, GGV: 13,
};
const DETERMINED_ENERGY: Record<string, number> = { AMI: 0, AZI: 1 };
const SOURCE: Record<string, number> = {
  ACTUAL: 0, ESTIMATED: 1, CORRECTED: 2, MANUAL: 3, UNDEFINED: 4,
};
...
function convertConnectionMetadata(m: ConnectionMetadata | undefined | null): unknown {
  ...
  return {
    ...
    determinedEnergyConsumption: toDeterminedEnergy(m.determinedEnergyConsumption),
  };
}
...
function convertIntervalReadings(r: IntervalReadings | undefined | null): unknown {
  if (r == null) return undefined;
  const values = (r.values ?? []).map((v) => ({
    ...SCHEMA_TAG,
 ...
 */
export function toPublishBody(payload: MeterPayload): { key: string; message: unknown } {
  return {
    key: payload.key,
